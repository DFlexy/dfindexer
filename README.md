...
Os arquivos de stack ficam em [`docker/`](docker/):
...
- ✅ Persistir dados do Redis em `docker/redis_data` (ao rodar de dentro de `docker/`)
...
O `docker/prowlarr.yml` usa `http://localhost:7006/` como URL base — compatível com essa stack em host.
...
1. Baixe o arquivo de configuração [`docker/prowlarr.yml`](docker/prowlarr.yml) neste repositório
