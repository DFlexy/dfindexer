...
from utils.http import get_aiohttp_proxy_connector
...
class TorrentEnricherAsync:
    ...
    async def _resolve_magnet_display_names(self, torrents: List[Dict]) -> None:
        ...
        async def fetch_one(torrent: Dict) -> None:
            ...
            if not info_hash:
                return
            try:
                from utils.semaphore import metadata_slot_async
    ...
    async def _fetch_metadata_batch(self, torrents: List[Dict]) -> None:
        from utils.semaphore import metadata_slot_async
        from cache.store import MetadataCache
        from utils.text import magnet_original_needs_raw_name, can_skip_metadata_fetch
            ...
            for result in results:
                ...
                if isinstance(result, tuple):
                    ...
                    if metadata:
                        torrent['_metadata'] = metadata
                        torrent['_metadata_fetched'] = True
                        from utils.text import upgrade_torrent_title_from_metadata
                        upgrade_torrent_title_from_metadata(torrent, metadata)
    ...
    async def _attach_peers(
        ...
        from utils.parsing import extract_trackers_from_magnet
        from utils.text import save_cross_data_to_redis
