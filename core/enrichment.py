...
def apply_imdb_fallback(torrents: List[Dict]) -> None:
    ...
    from cache.store import get_redis_client
    from cache.store import imdb_key, imdb_title_key
...
def save_metadata_name_to_cross_data(torrent: Dict, metadata: Dict) -> None:
    try:
        ...
        from utils.text import get_cross_data_from_redis, save_cross_data_to_redis
        from utils.text import _is_metadata_more_complete
        from utils.text import _normalize_metadata_name
...
def hydrate_torrent_from_cross_data(torrent: Dict, cross_data: Optional[Dict[str, Any]]) -> None:
    if not cross_data:
        return
    from utils.text import _looks_like_bludv_processed_release_name
