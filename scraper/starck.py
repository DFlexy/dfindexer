...
from utils.http import get_proxy_dict, get_proxy_url, is_proxy_local
from magnet.parser import MagnetParser
from utils.text import find_year_from_text, find_sizes_from_text
from utils.parsing import detect_audio_from_html
from utils.log import format_error, format_link_preview, ScraperLogContext
from utils.parsing import decode_data_u
...
SITE = SiteConfig(
    ...
    paginacao="page/{}/",
    seletores={
        "catalogo": "div.post-catalog, div.home.post-catalog",
        "card": ".item",
        "card_conteudo": "div.sub-item",
        "link_do_card_contem": "catalog",
        "post": "div.post",
        "capa": "div.capa",
        "titulo": ".post-description > h2",
        "ficha_paragrafos": ".post-description p",
        "titulo_alternativo": "h2.post-title",
        "botoes_download": ".post-buttons",
    },
)
...
def _invalidate_starck_gate_cache(redis, url: str) -> None:
    try:
        from cache.store import get_http_cache
        from cache.store import html_long_key, html_short_key
...
class StarckScraper(BaseScraper):
    ...
    def _extract_links_from_page(self, doc: BeautifulSoup) -> List[str]:
        ...
        catalog_div = doc.select_one(SITE.seletores["catalogo"])
        if not catalog_div:
            catalog_div = doc
        
        items = catalog_div.select(SITE.seletores["card"])
        
        for item in items:

            sub_item = item.select_one(SITE.seletores["card_conteudo"])
            if not sub_item:
                continue
            
            link_hint = SITE.seletores["link_do_card_contem"]
            all_links = sub_item.find_all('a', href=lambda h: h and link_hint in h)
            link_elem = None
    ...
    def _extract_search_results(self, doc: BeautifulSoup) -> List[str]:
        ...
        for item in catalog_div.select(SITE.seletores["card"]):
            ...
            link_hint = SITE.seletores["link_do_card_contem"]
            link_elem = sub_item.find('a', href=lambda h: h and link_hint in h, title=lambda t: t and t.strip())
    ...
    def _get_torrents_from_page(self, link: str) -> List[Dict]:
        ...
        torrents = []
        post = doc.select_one(SITE.seletores["post"])
        if not post:
            self._log_structure_miss(absolute_link, SITE.seletores["post"])
            return []
        
        capa = post.select_one(SITE.seletores["capa"])
        if not capa:
            self._log_structure_miss(absolute_link, SITE.seletores["capa"])
            return []
        
        page_title = ''
        title_elem = capa.select_one(SITE.seletores["titulo"])
        if title_elem:
            page_title = title_elem.get_text(strip=True)
        
        original_title = ''
        for p in capa.select(SITE.seletores["ficha_paragrafos"]):
        ...
        if not title_translated_processed:
            post_title_elem = capa.select_one(SITE.seletores["titulo_alternativo"])
            if post_title_elem:
        ...
        buttons_root = post.select_one(SITE.seletores["botoes_download"]) or post
