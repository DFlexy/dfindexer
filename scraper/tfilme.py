...
SITE = SiteConfig(
    ...
    url="https://torrentdosfilmes-v2.xyz/",
    caminho_busca="?s=",
    paginacao="category/dublado/page/{}/",
    seletores={
        "card": "div.post",
        "link_do_card": "div.title > a",
        "classe_serie": "blue",
        "classe_filme": "green",
        "bloco_secao": "titleGeral",
        "titulo_secao_filmes": "Últimos Filmes Adicionados",
        "titulo_secao_series": "Últimas Séries Adicionadas",
        "artigo": "article",
        "bloco_titulo": "div.title",
        "conteudo": "div.content",
        "paragrafos_conteudo": "div.content p",
    },
)
...
from utils.parsing import extract_labeled_value as _extract_labeled_value
from utils.parsing import extract_labeled_value_from_text as _extract_labeled_value_from_text
...
class TfilmeScraper(BaseScraper):
    ...
    def _extract_links_from_page(self, doc: BeautifulSoup) -> Tuple[List[str], List[str]]:
        ...
        for h3 in doc.find_all('h3'):
            if h3.get_text(strip=True) == SITE.seletores["titulo_secao_filmes"]:
                filmes_h3 = h3
                break
        
        if filmes_h3:
            title_geral_filmes = filmes_h3.find_parent('div', class_=SITE.seletores["bloco_secao"])
            if title_geral_filmes:
                current = title_geral_filmes.find_next_sibling()
                while current:
                    if current.name == 'div' and SITE.seletores["bloco_secao"] in current.get('class', []):
                        break
                    if current.name == 'div' and 'post' in current.get('class', []) and SITE.seletores["classe_filme"] in current.get('class', []):
                        link_elem = current.select_one(SITE.seletores["link_do_card"])
                        if link_elem:
        ...
        series_h3 = None
        for h3 in doc.find_all('h3'):
            if h3.get_text(strip=True) == SITE.seletores["titulo_secao_series"]:
                series_h3 = h3
                break
        
        if series_h3:
            title_geral_series = series_h3.find_parent('div', class_=SITE.seletores["bloco_secao"])
            if title_geral_series:
                current = title_geral_series.find_next_sibling()
                while current:
                    if current.name == 'div' and SITE.seletores["bloco_secao"] in current.get('class', []):
                        break
                    if current.name == 'div' and 'post' in current.get('class', []) and SITE.seletores["classe_serie"] in current.get('class', []):
                        link_elem = current.select_one(SITE.seletores["link_do_card"])
                        if link_elem:
                            href = link_elem.get('href')
                            if href:
    ...
    def _collect_post_links(self, doc: BeautifulSoup) -> Tuple[List[str], List[str]]:
        ...
        for item in doc.select(SITE.seletores["card"]):
            link_elem = item.select_one(SITE.seletores["link_do_card"]) or item.select_one('a[href]')
            if not link_elem:
                continue
            ...
            if SITE.seletores["classe_serie"] in classes:
                series_links.append(href)
            else:
                filmes_links.append(href)
    ...
    def _collect_magnet_links(self, doc: BeautifulSoup, article: BeautifulSoup) -> List[str]:
        ...
        text_content = article.select_one(SITE.seletores["conteudo"])
        if text_content:
    ...
    def _get_torrents_from_page(self, link: str) -> List[Dict]:
        ...
        torrents = []
        article = doc.select_one(SITE.seletores["artigo"])
        if not article:
            self._log_structure_miss(absolute_link, SITE.seletores["artigo"])
            return []
        
        page_title = ''
        title_div = article.select_one(SITE.seletores["bloco_titulo"])
        if title_div:
        ...
        for content_div in article.select(SITE.seletores["conteudo"]):
        ...
        content_div = article.select_one(SITE.seletores["conteudo"])
        idioma = ''
        ...
        if not audio_info:
            for p in article.select(SITE.seletores["paragrafos_conteudo"]):
                text = p.get_text()
                html_content = str(p)
                ...
                if not audio_info:
                    from utils.parsing import detect_audio_from_html
